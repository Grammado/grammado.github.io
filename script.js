...
function registerAttempt() {
  ...
  const gender_female = document.getElementById("mulher");
  const gender_male = document.getElementById("homem");
  ...
  if (pass_r.test(password) == false)
    return showError("A senha deve conter de 6 a 18 caracteres");

  if (password != passwordConfirm) {
    return showError("As senhas informadas não são iguais");
  }

  if (gender_male.checked == false && gender_female.checked == false)
    return showError("Escolha um sexo");

  if (getSkin == -1) return showError("Escolha uma skin inicial");
